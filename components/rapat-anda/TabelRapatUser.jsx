...
const TabelRapatUser = () => {
  ...
  const { data: rapats, error } = useSWR(
    cookies.token
      ? [
          ...
          }/rapats?_limit=${limit}&_start=${start}&_sort=${sortby}&user=${
            jwt_decode(cookies.token).id
          }&nama_contains=${search}`,
          cookies.token,
        ]
      : null
  );
  ...
  const { data: totalRapat } = useSWR(
    cookies.token
      ? [
          `${process.env.NEXT_PUBLIC_URL}/rapats/count?user=${
            jwt_decode(cookies.token).id
          }`,
          cookies.token,
        ]
